...
One recorded response per provider. The normaliser suites in
`../__tests__/` read these instead of calling the live APIs, which is what
makes `pnpm test` deterministic, offline, and fast enough to run on every
commit.
