...
import { CitationOptions, downloadCitation, generateCitation, generateCitationsBatch } from './citations';
...
export function generateBibTeXBatch(records: OARecord[], options?: Partial<CitationOptions>): string {
  return generateCitationsBatch(records, { format: 'bibtex', ...options });
}
