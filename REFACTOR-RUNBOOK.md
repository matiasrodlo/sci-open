...
- [x] **Ranking and enrichment salvaged** to `docs/salvage/`, with headers recording what to fix on the way back in. ~~Kept for phases 06 and 09.~~ **Both phases landed, so the salvage was deleted — 2026-08-30.** `mixResultsBySource` and `calculateRelevanceScore` became rank fusion plus `qualityScore` in `orchestrator/rank.ts`; `mergeCrossrefData` and `mergeUnpaywallData` became the Crossref and Unpaywall authorities. The whole 1,275-line original remains one command away: `git show 56817406:apps/api/src/lib/search-pipeline.ts`.
...
- [x] A cached search returns identical abstracts to a fresh one. Reconstruction is deterministic and tested in `providers/openalex/__tests__/normalize.test.ts` — the recorded abstracts rebuild, a `null` index yields no abstract, and anything that is not an index answers `undefined` rather than throwing. (This box originally cited an `abstract.test.ts` that never existed; the coverage does, and moved with the provider.) Phase 10 made the cache store serialised values and parse on read — so a cached reader gets a fresh object by construction and cannot diverge from a fresh one at all.
